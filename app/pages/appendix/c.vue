...
// ===== Focus mode =====
const { isFocused, currentIndex, currentStep, isVisible, focusAnchor, next: nextStep, prev: prevStep, showAll, steps } = useFocusMode(getSteps('app-c'))
...
const keyToAnchor = (i: number) => 'sec-app-c-' + (i + 1)
...
const saveAndNext = async () => { isSavingNext.value = true; await new Promise(r => setTimeout(r, 1000)); isSavingNext.value = false; router.push({ path: '/appendix/d', query: { id: programId.value } }) }
</script>

<template>
  <div class="page-shell">
    <form @submit.prevent class="w-full">
      <div class="crumb"><span>เล่มหลักสูตร</span> › <b>ภาคผนวก</b> › <b class="text-[#1B2A4A] font-semibold">ภาคผนวก ค.</b></div>
      <div class="doc-head"><div class="doc-eyebrow">ภาคผนวก ค.</div><h1 class="doc-title">สำเนาคำสั่งแต่งตั้งคณะกรรมการพัฒนาหลักสูตร</h1></div>
      ...
      <div v-if="!isFocused" class="toc-card">
        ...
        <div class="grid grid-cols-1 gap-[4px_18px]">
          <div class="toc-item" :class="{ 'filled': doneState[0] }" @click="focusAnchor('sec-app-c-1')">
            <div class="toc-dot"></div><span class="toc-num">ค.1</span><span class="lbl">{{ sectionTitles[0] }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="paper-card">
        <section class="topic-sec" id="sec-app-c-1" v-show="isVisible('sec-app-c-1')">
          <div class="sec-head">
      ...
      <div class="page-footer">
        <button type="button" @click="router.push(`/appendix/b?id=${programId || ''}`)" class="nav-btn">← <span>ภาคผนวก ข. รหัสวิชา/ปริญญา</span></button>
