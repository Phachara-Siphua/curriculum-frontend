...
const saveAndNext = async () => { 
  ...
  router.push({ path: '/appendix/e', query: { id: programId.value } })
}
...
<template>
  <div class="page-shell">
    <form @submit.prevent class="w-full">
      ...
      <!-- Breadcrumb & Title -->
      <div class="crumb">
        <span>เล่มหลักสูตร</span> › <b>ภาคผนวก</b> › <b class="text-[#1B2A4A] font-semibold">ภาคผนวก ง.</b>
      </div>
      <div class="doc-head">
        <div class="doc-eyebrow">ภาคผนวก ง.</div>
        <h1 class="doc-title">ระเบียบมหาวิทยาลัยเทคโนโลยีพระจอมเกล้าพระนครเหนือ ว่าด้วยการศึกษาระดับปริญญาบัณฑิต</h1>
      </div>
      ...
      <div class="toc-card">
        ...
        <div class="grid grid-cols-1 gap-[4px_18px]">
          <div class="toc-item" :class="{ 'filled': doneState[0] }" @click="scrollToSec('app-ง-1')">
            <div class="toc-dot"></div>
            <span class="toc-num">ง.1</span>
      ...
      <div class="paper-card">
        ...
        <section class="topic-sec" id="app-ง-1">
          <div class="sec-head">
      ...
      <div class="page-footer">
        <button type="button" @click="router.push(`/appendix/c?id=${programId || ''}`)" class="nav-btn">
          ← <span>ภาคผนวก ค. คำสั่งแต่งตั้ง</span>
        </button>
        <div class="flex flex-col md:flex-row gap-3">
          ...
          <button type="button" @click="saveAndNext()" :disabled="isSavingNext" class="btn-brass force-white-btn" style="border:none;border-radius:8px;padding:10px 16px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:8px;cursor:pointer;">
            <span style="color:#ffffff !important;">{{ isSavingNext ? 'กำลังบันทึก...' : 'ภาคผนวก จ. ผลงานวิชาการของอาจารย์' }}</span> <UIcon name="i-heroicons-arrow-right" class="w-4 h-4 text-white" />
          </button>
        </div>
      </div>
